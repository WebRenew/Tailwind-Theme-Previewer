...
export function hexToOklch(hex: string): string {
  ...
  const lightness = 0.2104542553 * lCube + 0.7936177850 * mCube - 0.0040720468 * sCube
...
export type ColorFormat = 'hex' | 'oklch' | 'hsla'
...
export function getTailwindColor(colorName: TailwindColorName, shade: TailwindColorShade): string {
  return TAILWIND_COLORS[colorName][shade]
}

export function parseTailwindColor(value: string): { colorName: TailwindColorName; shade: TailwindColorShade } | null {
  const match = value.match(/^(\w+)-(\d+)$/)
  if (!match) return null
  
  const [, colorName, shade] = match
  if (colorName in TAILWIND_COLORS && (shade === '50' || shade === '100' || shade === '200' || shade === '300' || shade === '400' || shade === '500' || shade === '600' || shade === '700' || shade === '800' || shade === '900' || shade === '950')) {
    return { colorName: colorName as TailwindColorName, shade }
  }
